...
void games_in_flash(){
	...
	switch(selected){
	case 0:
		load_flash_game((const uint8_t*)sokoban,SOKOBAN_SIZE);
		text_scroller((const uint8_t*)sokoban_info,4);
		break;
	case 1:
		load_flash_game((const uint8_t*)spacefig,SPACEFIG_SIZE);
		text_scroller((const uint8_t*)spacefig_info,4);
		break;
	case 2:
